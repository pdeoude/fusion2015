...
The MDP approach has been extended to include multi-agent decision-making, \ie MMDP and MPOMDP. In these cases, the action space $\mathcal{A}$ represents the joint-actions $\A_{m} = \A_1\times\ldots\times\A_n$ of all $n$ agents under consideration. Where solving an MDP can be performed in polynomial time, {\red cite} solving multi-agent problems suffer from the fact that their action space increases exponentially with the number of agents.
...
\section{Dynamic Multi-Agent Decision-Making}
\label{sec:ma-dec-mak}

Using the described definition of the smugglers case, our goal is to find a joint strategy that minimizes the probability of escape for the smuggler. Based on the complexity and dynamic nature of the problem, constructing a plan off-line is not a feasible approach {\red cite?}. However, on-line planning offers the capability of establishing a plan that i) immediately generates an ongoing strategy, ii) which improves over time, and iii) can cope with incoming observations, sudden constraints, and changing situations.
...
\begin{center}
 ...
 \caption{Architecture of multi-layered planner.}\label{fig:searchtree}
\end{center}
\end{figure}
