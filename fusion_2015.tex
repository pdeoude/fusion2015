...
Using the described definition of the smuggler problem, our goal is to find a course of action and joint strategy which minimizes the probability of escape for the smuggler. Based on the complexity and dynamic nature of the problem, constructing plans off-line is not a feasible approach {\red cite?}. On-line planning offers the capability of constructing a plan that i) generates an immediate strategy, ii) which improves over time and iii) can cope with incoming observations, sudden constraints, and changing situations.
