...
In the defined state representation, the locations of the friendly {\red define friendly/hostile somewhere, perhaps bluefor and opfor are better terms? rik: I think friendly fits the notion here quite well. Blue force is a pretty militarized term in my opinion} agents are assumed to be known at each point in time. This assumption is based on the availability of GPS localization and wireless communication techniques. However, the location of the smuggler is unknown to the agents and therefore our planner. Given that the goal of the multi-agent plan is to locate, track and capture the smuggler, respectively, the combined set of agents needs to maintain a belief over the current location of the smuggler's go-fast.
